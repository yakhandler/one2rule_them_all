...
- **Conflicts block (exit code 2).** Same name + different definition/content → the engine refuses to guess, prints both, writes nothing. Resolve via `--prefer <keys>` (priority list picks a winner) or by editing one side to match. Exit codes: `0` ok, `2` unresolved conflicts, `3` ran but some files were unreadable.
